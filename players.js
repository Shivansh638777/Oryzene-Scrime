...
const DATA = path.join(__dirname, '..', 'data', 'players.json');
...
module.exports = (req,res) => {
  ...
  if(req.method === 'POST'){
    let body=''; req.on('data',c=>body+=c); req.on('end',()=>{
      try{
        const { name, email, phone, game, nda } = JSON.parse(body||'{}');
        if(!nda) return res.status(400).json({message:'NDA required'});
        const list = read(); const item = { id: Date.now(), name, email, phone, game, approved:false };
  ...
  if(req.method === 'PUT'){
    ...
    let body=''; req.on('data',c=>body+=c); req.on('end',()=>{
      try{
        ...
        const list = read(); const idx = list.findIndex(x=>String(x.id)===String(id));
