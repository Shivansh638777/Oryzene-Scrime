...
const DATA = path.join(__dirname, '..', 'data', 'sponsors.json');
...
module.exports = (req,res) => {
  ...
  if(req.method === 'POST'){
    let body=''; req.on('data',c=>body+=c); req.on('end',()=>{
      try{
        const { brand, contact, email, message } = JSON.parse(body||'{}');
        const list = read(); const item = { id: Date.now(), brand, contact, email, message, approved:false };
